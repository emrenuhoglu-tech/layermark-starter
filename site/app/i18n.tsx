...
const translations = {
  tr: {
    ...
    // Vanilla vs Layermark comparison (homepage)
    'vs.tag': 'VANILLA CLAUDE CODE vs LAYERMARK-STARTER',
    'vs.title': 'Aynı Claude Code modeli, farklı framework',
    'vs.subtitle':
      "Boş klasörde `claude` çalıştırırsan Claude Code'u alırsın — ama doctrine yok, skill yok, agent yok, kategori-aware risk profili yok. Starter bu scaffold'u distile, source-cited halde pre-loaded getirir.",
    'vs.vanilla.label': 'BOŞ KLASÖR + claude',
    'vs.vanilla.title': 'Vanilla Claude Code',
    'vs.vanilla.r1': "CLAUDE.md yok — Claude her session'da projeyi sıfırdan keşfeder",
    'vs.vanilla.r2': "/init çalıştırırsan: 200+ satır generic boilerplate (Anthropic'in kendi Doctrine #10 ihlali)",
    'vs.vanilla.r3': '0 skill — /grill-me, /failing-test-as-prompt, /verify-agent-output, /agent-approval, /suspend, /resume, /project-advisor yok',
    'vs.vanilla.r4': "Subagent template yok — casual istek structured prompt'a dönüşmez, audit/security pass yok",
    'vs.vanilla.r5': 'Domain-blind — finans projesi de oyun projesi de aynı muameleyi görür',
    'vs.vanilla.r6': 'Verification self-reported — multi-grader rubric, eval-as-CI-gate yok',
    'vs.vanilla.r7': 'Multi-agent için scaffold yok — orchestrator-safety, saga, circuit-breaker patterns sıfırdan',
    'vs.starter.label': 'SETUP_STARTER + claude',
    ...
    'vs.starter.r7': 'Phase 0.5 multi-agent gate + 02-memory/orchestrator-safety.md (saga + circuit-breaker) + Doctrine #15-20 production opt-in',
    'vs.footer.1': 'Her satırın kaynağı + kanıt commit hash\'i',
    'vs.footer.link': '/why sayfasında',
    'vs.footer.2': '— "trust me bro" değil, source-cited.',
    ...
    // Setup Service tier — RESEARCH PHASE (validated demand yok, pilot kalibrasyon)
    'service.tag': 'HİZMET KATMANI · ARAŞTIRMA FAZI',
    'service.title': 'Setup service — pilot fazında, ücretsiz keşif çağrısı',
    'service.subtitle': 'OSS starter ücretsiz kalıyor (her zaman). Bunun yanına "wizard\'ı kendim çalıştırmak istemiyorum, kurun ve teslim edin" senaryosu için ücretli bir hizmet katmanı tasarlıyoruz — ama henüz pilot fazındayız. Sözleşmeli teslimat, sabit fiyat ve garanti yok; ihtiyacını anlamak için 15 dakikalık ücretsiz keşif çağrısı var.',

    'service.pkg.title': 'Pilot setup — birlikte tanımlıyoruz',
    'service.pkg.b1': 'Keşif çağrısında ihtiyacını anlıyoruz (kategori, scope, beklenti)',
    'service.pkg.b2': 'Birlikte uygunsa: domain\'ine özel skill + agent prototip iş tanımı çıkarıyoruz',
    'service.pkg.b3': 'Pilot fiyatlandırma — ilk 3 müşteri %50 indirim, paket birlikte kalibre ediliyor',
    'service.pkg.b4': 'Açık iletişim: yapamadığımız şeyi söylüyoruz (mesela "5 saat tasarruf garanti" gibi vaadin altyapısı henüz yok)',
    'service.pkg.b5': 'Pilot phase: SLA yok, best-effort destek (issue gelirse 1 hafta içinde dönüyoruz, formal taahhüt değil)',
    'service.pkg.timeLabel': 'Faz',
    'service.pkg.time': 'Pilot · pilot müşteri arıyor',
    'service.pkg.priceLabel': 'Fiyat',
    'service.pkg.price': 'Keşif sonrası teklif',
    'service.pkg.retainerLabel': 'Retainer',
    'service.pkg.retainer': 'Henüz tanımlı değil — pilot sonrası',

    'service.guarantee.tag': 'NE VAR · NE YOK',
    'service.guarantee.title': 'Şeffaf konuşalım',
    'service.guarantee.desc': 'Şu an 0 ücretli müşteri var. "5 saat/hafta tasarruf garanti" gibi metrik altyapımız hazır değil. Sertçe vaad etmiyoruz. Yapabildiğimiz: starter\'ı doğru kurmak, doctrine\'leri doğru bağlamak, ilk hafta seni ayakta tutmak. Yapamadığımızda söylüyoruz. Pilot fazında risk paylaşımı: ilk müşterilere %50 indirim, paket fiyat birlikte kalibre ediliyor.',

    'service.who.title': 'Kimler için bakıyoruz',
    'service.who.b1': 'Non-tech ekipler (kod yazmadan Claude Code\'u verimli kullanmak isteyen)',
    'service.who.b2': 'HIGH-RISK kategorilerde çalışanlar (finans, hukuk) — auditor agent\'larıyla pilot ilgisi olan',
    'service.who.b3': 'Mevcut Claude Code projesi olup layermark-doctrine audit\'inden geçirmek isteyenler',

    'service.cta': '📞 Keşif çağrısı (15 dk · ücretsiz · taahhütsüz)',
    'service.cta.note': 'Email gönder → 24 saat içinde dönüyoruz · Pilot fazı, sözleşme yok',

    // Research notes — pack hipotezleri + tutorial backlog (DRAFT)
    'research.tag': 'ARAŞTIRMA NOTLARI · TASLAK',
    'research.title': 'Pilot müşterilerle birlikte tanımlanacak',
    'research.subtitle': 'Service tier\'ın iki bileşeni hâlâ açık taslak: (1) kategori başına otomasyon paketleri, (2) "command center" gibi pattern tutorial\'ları. Aşağıdakiler **hipotez** — sabit ürün değil. Pilot müşteriler ile gerçek ihtiyaca göre kalibre ediliyor.',

    'research.packs.tag': 'PAKET HİPOTEZLERİ',
    'research.packs.title': '5 kategori · ~5 otomasyon hipotezi her birinde',
    'research.packs.lead': 'EdgeAutomate\'in modeli (5 sabit otomasyon $3,997) ilham — ama biz layermark wedge\'inde kategoriye göre kalibre ediyoruz. Liste kapalı değil, pilot ile değişebilir.',
    'research.packs.p1.name': 'Otomasyon (operations)',
    'research.packs.p1.ideas': 'WhatsApp/email auto-reply · daily digest · calendar booking · lead capture form → CRM · cron-based reporting',
    'research.packs.p2.name': 'Finans (HIGH-RISK)',
    'research.packs.p2.ideas': 'Invoice extraction · P&L tracker · expense categorizer · audit trail (immutable ledger) · KVKK-compliant logging',
    'research.packs.p3.name': 'Hukuk (HIGH-RISK)',
    'research.packs.p3.ideas': 'Contract clause extractor · jurisdiction-aware disclaimer generator · regulation citer (eur-lex / kvkk.gov.tr) · PII redactor · case-law lookup',
    'research.packs.p4.name': 'Pazarlama',
    'research.packs.p4.ideas': 'Content draft · channel cross-poster · A/B copy variant · brand-voice glossary lint · weekly performance digest',
    'research.packs.p5.name': 'Eğitim',
    'research.packs.p5.ideas': 'Course outliner · quiz generator · student progress tracker · cross-reference linter · citation source-checker',

    'research.tutorials.tag': 'TUTORIAL BACKLOG',
    'research.tutorials.title': 'İlk pilotlardan sonra yazılacak',
    'research.tutorials.lead': 'Tutorial yazımı blog/SEO için değil — pilot müşterilerin tekrar eden sorularına cevap olarak. Aşağıdaki başlıklar şu an scope\'ta, ilk 3 pilot tamamlanmadan ship edilmeyecek.',
    'research.tutorials.t1.name': 'Command center pattern (APEX-style)',
    'research.tutorials.t1.desc': 'Layermark + tek-sayfa ops dashboard (multi-room metaphor). 9-agent orchestrator için ortak panel. Kaynak: apex-command-seven.vercel.app referansı.',
    'research.tutorials.t1.status': 'OUTLINE',
    'research.tutorials.t2.name': 'HIGH-RISK kategori migration',
    'research.tutorials.t2.desc': 'Mevcut Claude Code projesi → finance/legal kategori upgrade. Production doctrine + auditor agent + permission tightening adım adım.',
    'research.tutorials.t2.status': 'PLANNED',
    'research.tutorials.t3.name': 'Hook layer customization',
    'research.tutorials.t3.desc': 'Pocock Memento doctrine\'i ekibinin workflow\'una göre özelleştirme — activity_tracker.py + session_snapshot.py extension pattern\'leri.',
    'research.tutorials.t3.status': 'PLANNED',
    'research.tutorials.cta': 'Bu tutorial\'lardan birine somut ihtiyacın varsa keşif çağrısında söyle — ilk pilotun konusu o olabilir.',

    'research.footer': 'Doctrine #9 (rules emerge from friction): hipotezler pilot müşterilerin gerçek ihtiyacıyla kesişene kadar ürün değil — taslak. Validated demand olmadan paket sabitlemiyoruz.',
    ...
    'start.value.link': 'Her birinin kaynağı + kanıtı /why sayfasında →',
  },
  en: {
    'nav.how': 'How',
    'nav.what': 'What ships',
    ...
    // Vanilla vs Layermark comparison (homepage)
    'vs.tag': 'VANILLA CLAUDE CODE vs LAYERMARK-STARTER',
    'vs.title': 'Same Claude Code model, different framework',
    'vs.subtitle':
      "Run `claude` in an empty folder and you get Claude Code — but no doctrine, no skills, no agents, no category-aware risk profile. The starter pre-loads that scaffold, source-cited.",
    'vs.vanilla.label': 'EMPTY FOLDER + claude',
    'vs.vanilla.title': 'Vanilla Claude Code',
    'vs.vanilla.r1': 'No CLAUDE.md — Claude rediscovers the project every session',
    'vs.vanilla.r2': "Run /init: 200+ lines of generic boilerplate (violates Anthropic's own Doctrine #10)",
    'vs.vanilla.r3': '0 skills — no /grill-me, /failing-test-as-prompt, /verify-agent-output, /agent-approval, /suspend, /resume, /project-advisor',
    'vs.vanilla.r4': "No subagent template — casual requests don't convert to structured prompts; no audit/security pass",
    'vs.vanilla.r5': 'Domain-blind — a finance project gets the same treatment as a game',
    'vs.vanilla.r6': 'Verification self-reported — no multi-grader rubric, no eval-as-CI-gate',
    'vs.vanilla.r7': 'No multi-agent scaffold — orchestrator-safety, saga, circuit-breaker patterns from scratch',
    'vs.starter.label': 'SETUP_STARTER + claude',
    'vs.starter.title': 'layermark-starter + Claude',
    ...
    'vs.starter.r6': 'Doctrine #6 (Verification) + /verify-agent-output skill — multi-grader rubric, eval-awareness defense, red-team primitive',
    'vs.starter.r7': 'Phase 0.5 multi-agent gate + 02-memory/orchestrator-safety.md (saga + circuit-breaker) + Doctrines #15-20 production opt-in',
    'vs.footer.1': 'Every line\'s source + proof commit hash',
    'vs.footer.link': 'on /why',
    'vs.footer.2': '— not "trust me bro", source-cited.',
    ...
    // Setup Service tier — RESEARCH PHASE
    'service.tag': 'SERVICE TIER · RESEARCH PHASE',
    'service.title': 'Setup service — in pilot phase, free discovery call',
    'service.subtitle': 'The OSS starter stays free (always). Alongside it, we are designing a paid service tier for users who say "I don\'t want to run the wizard myself, just set it up and hand it over" — but we are still in pilot phase. No fixed pricing, no contractual delivery, no guarantees. Just a 15-minute free discovery call to understand your needs.',

    'service.pkg.title': 'Pilot setup — defined together',
    'service.pkg.b1': 'In the discovery call we understand your need (category, scope, expectations)',
    'service.pkg.b2': 'If a fit: we draft a domain-specific skill + agent prototype scope',
    'service.pkg.b3': 'Pilot pricing — first 3 customers get 50% discount; package calibrated together',
    'service.pkg.b4': 'Open communication: we tell you what we cannot deliver yet (e.g. "5 hours saved/week" measurement infrastructure is not ready)',
    'service.pkg.b5': 'Pilot phase: no SLA, best-effort support (issues addressed within 1 week, no formal commitment)',
    'service.pkg.timeLabel': 'Phase',
    'service.pkg.time': 'Pilot · seeking first customers',
    'service.pkg.priceLabel': 'Price',
    'service.pkg.price': 'Quoted after discovery',
    'service.pkg.retainerLabel': 'Retainer',
    'service.pkg.retainer': 'Not defined yet — post-pilot',

    'service.guarantee.tag': "WHAT'S THERE · WHAT ISN'T",
    'service.guarantee.title': "Let's be transparent",
    'service.guarantee.desc': 'Right now: 0 paid customers. We do not have measurement infrastructure for promises like "5 hours saved per week". We are not making hard guarantees. What we can do: set the starter up correctly, wire the doctrines, keep you on rails for the first week. When we cannot, we say so. Pilot phase risk-sharing: first customers get 50% discount; pricing calibrated together.',

    'service.who.title': 'Who we are looking at',
    'service.who.b1': 'Non-technical teams (want efficient Claude Code usage without writing code)',
    'service.who.b2': 'HIGH-RISK categories (finance, legal) — interested in piloting auditor agents',
    'service.who.b3': 'Teams with existing Claude Code projects who want a layermark-doctrine audit',

    'service.cta': '📞 Discovery call (15 min · free · no commitment)',
    'service.cta.note': 'Send an email → reply within 24 hours · pilot phase, no contracts',

    // Research notes (DRAFT)
    'research.tag': 'RESEARCH NOTES · DRAFT',
    'research.title': 'Defined together with pilot customers',
    'research.subtitle': 'Two service-tier components are still open drafts: (1) per-category automation packs, (2) pattern tutorials (e.g. "command center"). The lists below are **hypotheses** — not fixed products. They will be calibrated with pilot customers based on real needs.',

    'research.packs.tag': 'PACK HYPOTHESES',
    'research.packs.title': '5 categories · ~5 automation hypotheses each',
    'research.packs.lead': 'EdgeAutomate\'s model (5 fixed automations for $3,997) is inspiration — but we calibrate per-category to fit the layermark wedge. The list isn\'t closed; pilots can shift it.',
    'research.packs.p1.name': 'Operations',
    'research.packs.p1.ideas': 'WhatsApp/email auto-reply · daily digest · calendar booking · lead capture → CRM · cron-based reporting',
    'research.packs.p2.name': 'Finance (HIGH-RISK)',
    'research.packs.p2.ideas': 'Invoice extraction · P&L tracker · expense categorizer · audit trail (immutable ledger) · GDPR/KVKK-compliant logging',
    'research.packs.p3.name': 'Legal (HIGH-RISK)',
    'research.packs.p3.ideas': 'Contract clause extractor · jurisdiction-aware disclaimer generator · regulation citer (eur-lex / kvkk.gov.tr) · PII redactor · case-law lookup',
    'research.packs.p4.name': 'Marketing',
    'research.packs.p4.ideas': 'Content draft · channel cross-poster · A/B copy variant · brand-voice glossary lint · weekly performance digest',
    'research.packs.p5.name': 'Education',
    'research.packs.p5.ideas': 'Course outliner · quiz generator · student progress tracker · cross-reference linter · citation source-checker',

    'research.tutorials.tag': 'TUTORIAL BACKLOG',
    'research.tutorials.title': 'Written after the first pilots',
    'research.tutorials.lead': 'Tutorials are not for blog/SEO — they answer recurring questions from pilot customers. Titles below are in scope but won\'t ship until the first 3 pilots are complete.',
    'research.tutorials.t1.name': 'Command center pattern (APEX-style)',
    'research.tutorials.t1.desc': 'Layermark + single-page ops dashboard (multi-room metaphor). Shared panel for a 9-agent orchestrator. Reference: apex-command-seven.vercel.app.',
    'research.tutorials.t1.status': 'OUTLINE',
    'research.tutorials.t2.name': 'HIGH-RISK category migration',
    'research.tutorials.t2.desc': 'Existing Claude Code project → finance/legal category upgrade. Production doctrine + auditor agent + permission tightening, step by step.',
    'research.tutorials.t2.status': 'PLANNED',
    'research.tutorials.t3.name': 'Hook layer customization',
    'research.tutorials.t3.desc': 'Tailoring Pocock\'s Memento doctrine to your team\'s workflow — activity_tracker.py + session_snapshot.py extension patterns.',
    'research.tutorials.t3.status': 'PLANNED',
    'research.tutorials.cta': 'If you have a concrete need for one of these tutorials, mention it in the discovery call — it could be the topic of your pilot.',

    'research.footer': 'Doctrine #9 (rules emerge from friction): hypotheses are not products until they intersect with real pilot needs — they stay draft. We don\'t fix the pack until validated demand exists.',
    ...
    'start.value.link': 'Each one\'s source + evidence on /why →',
  },
} as const;
